...
import Nuko.Typer.Env        (MonadTyper, addTyKind, tsFields, addTy, tsConstructors, TyInfo (IsTySyn, IsTyDef))
...
inferTypeDecl (TypeDecl name args body) initData = do
  ...
  where
    ...
    inferField (fieldName, ty) = do
      ...
      pure (fieldName, (foldr (\(n, _) b -> TyForall n (\_ -> b)) inferedTy initData.itBindings))
